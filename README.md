...
- **桌面健康检查**:设置页一键检查 FFmpeg/FFprobe、解析工具、10 类模型、LLM 连通/鉴权/路由、磁盘、转写缓存、渲染缓存与多模态证据索引;缺少核心模型时显式准备,支持取消与断点续传;1GB 渲染缓存与 64MB 证据索引可分别安全清理
...
| v0.18 ~ v0.26(质量评测 · 字幕质量门 · 多模态/屏显文字证据 · 舒适竖屏构图 · 成片质检 · 智能画面校正 · HDR 安全转 SDR · 同轨分析 · 质量封面 · 语音安全切点) | ✅ 已发布 |
