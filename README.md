...
**[v0.12.0](https://github.com/xixihhhh/hotclip/releases/tag/v0.12.0)**(2026-08-05)「挑得准、开场炸」:**爆点闪现(flash-forward)**——正片开始前先闪 0.3-1 秒全片最炸瞬间再切回,悬念开场手法(全网仅 0.04% 切片做了 visual hook);与高潮前置自动二选一,定位失败自动跳过。**AI 画面复核**——检测后对头部候选抽九宫格接触表让视觉模型过目:画面炸裂加分、信号候选画面死气降分、画面与标题货不对板标警告,看点写进推荐理由;本机 Ollama 免费,视觉设置填 API Key 即切云端模型。**精准切点**——导出前用 Paraformer 对选中片段二遍解码,一体化词级时间戳(±50ms,超过强制对齐)修正字幕、跳剪与切点,快档转写(SenseVoice)的时间戳短板补齐;对不上自动回退,绝不改坏。**钩子兑付校验**——钩子里承诺的数字必须真出现在片中,没兑付的写进质检报告,防「标题党切片」。**字幕现代化**——默认样式换「关键词高亮」(整句扫色的老卡拉OK已过时,降为备选样式);「逐词弹色」升级:当前词品牌色点亮+阻尼弹入,对齐 2026 逐词字幕主流形态。另修复桌面端 4 个从未生效的导出开关(降噪/高潮前置/合集/双画幅)。

<details>
<summary><b>历史版本</b>(v0.4.3 → v0.11.0)与里程碑一览</summary>
