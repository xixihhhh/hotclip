...
<details>
<summary><b>展开细节</b>:横屏转竖屏 · 去气口 · 降噪 · 音效 · BGM · 质检自修复 · 智能封面 · 高潮前置</summary>
...
**[v0.11.0](https://github.com/xixihhhh/hotclip/releases/tag/v0.11.0)**(2026-08-05)「切完就能发」:**平台发布包**——导出后按平台整理齐套素材到「发布包/」,每平台一个文件夹:视频(硬链接不占双份磁盘)+ 按平台画幅重裁的封面(小红书 3:4、B站 16:10、视频号 6:7)+ 按平台上限适配的文案(小红书标题硬截 20 字、话题数各按平台),manifest 记录适配了什么——同一批片发 N 个平台,不用再逐平台手改规格。**一片多版**——同一切片一次出 2-3 版差异化包装:不同钩子角度的贴片标题、开场悬念句、发布文案,封面自动抓下一个响度峰;多账号分发靠真差异(2026 起平台判原创看「实质增量价值」,抽帧/镜像已被明确判搬运,这条路不做);变体在 clips.json 标 `variantOf`,合集与 EDL 自动只收原版。**音效打点与 BGM**——whoosh 卡拼接/高潮前置的硬切缝、「叮」卡全片情绪最高点、开场钩子上屏轻响,每条最多 3 个宁缺毋滥;BGM 自动循环铺满、音量压在人声之下、说话时自动闪避、结尾淡出——混音是独立后处理趟,视频画质零损失。另有**动态极简**字幕样式一档。

<details>
<summary><b>历史版本</b>(v0.4.3 → v0.10.1)与里程碑一览</summary>
