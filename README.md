...
**[v0.14.1](https://github.com/xixihhhh/hotclip/releases/tag/v0.14.1)**(2026-08-11)「想得再深、也得出片」:修复「找爆点失败:LLM 未返回内容」([#8](https://github.com/xixihhhh/hotclip/issues/8))——各平台的默认/热门模型如今多是「深度思考」型,思考过程也计入输出预算,长转写稿一思考就把 4000 token 预算全烧光、正文为空,换平台也躲不开。现在检测到「只有思考、没有正文」会自动换 16000 token 大预算重试一次,多数场景直接救活;个别网关把正文错放进 reasoning 字段的也兜底取回;仍然失败则按证据分因报错——思考模型烧完预算(指引换非思考版)/内容被安全审查拦截(指引换供应商或素材)/服务端偶发空响应(指引重试),每条都写明下一步。

<details>
<summary><b>历史版本</b>(v0.4.3 → v0.14.0)与里程碑一览</summary>
