...
- **发出去还能活下来**:变形度评分防判搬运、AIGC 标注、分发台账留证、多版反指纹——2026 分发规则的另一半答案
- **睡觉也在出片**:录播监听 7×24 盯住录播姬 / OBS 目录,录完自动切;CLI / MCP 让 Agent 一句话交付
...
- **分发台账与留证包**:每条切片对应的源片区间落 CSV 台账,可选保存前后 3 分钟源片切段——授权切片被质疑搬运时,证据链拿得出手
...
**开发中 · 将随下版发布**「弹幕这票,投得更准」:**抖音直播录制弹幕 .jsonl 兼容**(与 B 站录播姬 .xml 双格式自动发现,抖音主播的录播从此也有弹幕证据);**互动事件计权**(SC / 舰长 / 礼物 / 关注 / 点赞按档加权——花钱和动手的票比弹幕硬);**反刷屏**(同一人单窗贡献封顶,一个人刷一百条不算全场沸腾);**突发加成**(相对前一窗的热度跳升计入得分——爆点是「突然炸」,持续温热不误圈);**信号融合升级**(多路共振加成:音量+弹幕+笑声同秒共振才可信;窗口随内容伸缩:碎峰给短窗、持续高潮给长窗);**弹幕引导采样**(表情 / 语气这类贵模型的算力优先花在观众炸锅处)
