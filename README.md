...
- **本地逐字转写(三档可选)**:快速 SenseVoice(五语种,170MB)/ 均衡 Paraformer(中文更准,230MB)/ 最准 FireRedASR2(普通话/方言/中英混说,小红书开源,520MB)——全部本地运行、逐字时间戳、首次自动下载(国内镜像优先),模型不带标点的档位自动用标点模型回补;云端 ASR 档(需自带 Key)在路上
...
- **AI 复评质量门**:严格评审员二次盲评每条候选(前3秒钩子/是否断章取义/独立可看),弱候选透明标记默认不选——托管出的每条都过双重关
- **去录屏UI**:手机直播录屏的状态栏/固定UI/上下黑边,用时域方差自动检测并裁除(静止的是UI,会动的是内容),检测不到就不裁
- **气口跳剪**:自动剪掉说话间的停顿静音并拼接,字幕时间轴同步重映射——成片节奏像人剪的,不是机切
- **封面图 + 元数据导出**:每条切片附带封面 JPG(带字幕和标题贴片,平台直传)和 clips.json(标题/钩子/评分/评审意见/时间码/关键词),矩阵运营直接接 CMS
