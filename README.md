...
- **证据只算一次,越用越快**:响度、镜头、运动与可选画面扫描按源文件指纹进入本地证据索引;桌面、CLI、MCP、录播监听与 webhook 共同复用,算法或模型变化时自动失效重算
...
- **桌面健康检查**:设置页一键检查 FFmpeg/FFprobe、解析工具、9 类模型、LLM 连通/鉴权/路由、磁盘、转写缓存、渲染缓存与多模态证据索引;缺少核心模型时显式准备,支持取消与断点续传;1GB 渲染缓存与 64MB 证据索引可分别安全清理
...
| v0.18 ~ v0.19(质量评测 · 字幕质量门 · 多模态证据复用 · 运动曲线) | ✅ 已发布 |
