...
<details>
<summary><b>展开细节</b>:审阅台 · 安全区 · 发布包 · 一片多版 · 品牌模板 · 双画幅 · 合集 · EDL · 设置页</summary>
...
**[v0.14.0](https://github.com/xixihhhh/hotclip/releases/tag/v0.14.0)**(2026-08-09)「发得出去、活得下来」:2026 平台三条新规——搬运判定看变形度、收藏权重超四成、AI 内容强制标注——这一版整个对着「发出去不折号」做。**变形度评分**——把本次导出实际做的变形(字幕/跳剪/运镜/音效/BGM/封面/多版)汇总成一个分,出片前实时预估、导出回执落档,低于 40 黄牌提醒再开几项——像素级搬运是 2026 判搬运第一死因。**AI 封面双档**——按切片标题生成竖版大字封面:走量档 Seedream 约 ¥0.3/张、精品档 Nano Banana Pro 约 ¥1/张,落在成片旁与抓帧封面并存由你挑;真机实测中文大字零错字;复用 Atlas Key 零新增配置。**AI 配乐**——按直播品类生成版权安全纯音乐(MiniMax 约 ¥1/首,2 分半完整曲),生成完自动挂进 BGM 混音链(循环铺满/人声闪避/结尾淡出全复用)——平台查 BGM 版权越来越严,生成音乐没有第三方版权主张。**剪映草稿导出**——每条切片落一个剪映草稿夹,AI 切点连跳剪的每一刀都已铺进时间轴,拷进剪映草稿目录即开,精修不必从零开始。**说话人标签字幕**——对谈换人时行首加彩色「A:」「B:」标签,静音刷视频的观众不迷路。**实用密度加分**——步骤/清单/数字/金句密集的片段标「可收藏」并小幅加分,发布文案标题埋搜索词、CTA 引导收藏——收藏向内容才吃得到 7 天慢推流。**分发台账与留证包**——台账 CSV 记每条的源片区间(发布链接留空列),授权审核对账直接交表;可选把切片前后各 3 分钟源片流复制存档,有争议拿得出上下文。**AIGC 标注助手**——按平台写好标注文案与入口位置;单场出片超 5 条提示限产(单场刷屏本身就是搬运信号)。**多版反指纹**——一稿多版最后一版自动换 flash-forward 开场(变体连开场结构都不同),模板受控微扰(种子化字号/基线抖动)让批量出片没有量产指纹。**保留呼吸口**——跳剪可选留 0.25 秒自然停顿,不再句句贴脸。

<details>
<summary><b>历史版本</b>(v0.4.3 → v0.13.0)与里程碑一览</summary>
