...
async function grabFrame(ffmpeg: string, filePath: string, atSec: number): Promise<string> {
  try {
    const { stdout } = await execFileAsync(
      ffmpeg,
      ["-hide_banner", "-ss", atSec.toFixed(2), "-i", filePath, "-frames:v", "1", "-vf", "scale=200:-2", "-q:v", "6", "-f", "mjpeg", "pipe:1"],
...
export async function extractFilmstrip(filePath: string, durationSec: number, count = 8): Promise<string[]> {
  const ffmpeg = resolveFfmpegPath();
  const out: string[] = [];
  // 串行抽帧:seek 型单帧任务本身毫秒级,并发反而容易在机械盘上互相踩
  for (const t of filmstripTimes(durationSec, count)) {
    out.push(await grabFrame(ffmpeg, filePath, t));
  }
  return out;
}
