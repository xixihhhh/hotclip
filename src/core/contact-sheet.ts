 ...
 * 量级;同一张图也能落盘给人快速扫片。借鉴 claude-real-video 的接触表与
 * video-use「决策点才给图」的两层阅读思路。
 *
...
export function buildSheetArgs(
  ...
  const cells = times.map(
    (_t, i) => `[${i}:v]trim=end_frame=1,setpts=PTS-STARTPTS,scale=${w}:-2${label(i)}[f${i}]`
  );
