...
- In main, shipping with the next release: opening-hook burn-in (first 3 golden seconds), loudness normalization to the -14 LUFS social standard, **shot-snapped cut points** (TransNetV2 shot-boundary detection — cuts land on real shot changes), a **clip review workbench** (preview each clip in-app and drag cut points on a waveform timeline before exporting), and **brand style templates** (highlight color / font size / caption position / logo watermark — set once, applied to every clip)
