...
- In main, shipping with the next release: opening-hook burn-in (first 3 golden seconds), loudness normalization to the -14 LUFS social standard, **shot-snapped cut points** (TransNetV2 shot-boundary detection — cuts land on real shot changes), and a **clip review workbench** (preview each clip in-app and drag cut points on a waveform timeline before exporting)
