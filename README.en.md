...
**[v0.14.0](https://github.com/xixihhhh/hotclip/releases/tag/v0.14.0)** (2026-08-09) "Ship it, survive it": three 2026 platform rules — repost detection scores how much you *transformed*, saves carry >40% of ranking weight, AI content must be labeled — and this release is built for all three. **Transform score** — everything this export actually changed (captions / jump cuts / camera moves / SFX / BGM / covers / variants) rolls into one score, previewed live before export and written into the receipt; under 40 gets a yellow card telling you to switch on more transforms — pixel-level reposting is the #1 takedown cause in 2026. **Two-tier AI covers** — a vertical big-type cover generated from the clip title: volume tier Seedream (~$0.04/image) or premium tier Nano Banana Pro ($0.14/image), saved next to the frame-grab cover so you pick; live-tested with zero-typo Chinese headlines; reuses your Atlas key, zero new config. **AI BGM** — a copyright-safe instrumental generated per stream genre (MiniMax, ~$0.15 for a full 2.5-minute track), auto-wired into the existing BGM mix chain (looping, voice ducking, end fade) — generated music carries no third-party copyright claim. **JianYing draft export** — each clip lands as a JianYing draft folder with every AI cut (jump cuts included) already on the timeline; drop it into the drafts directory and fine-tune instead of starting over. **Speaker-tag captions** — colored "A:"/"B:" prefixes whenever the talker changes, so muted viewers can follow a conversation. **Utility density** — clips dense in steps, lists, numbers and quotables get flagged save-worthy with a small boost; publish copy embeds search terms and a save CTA — save-oriented content is what rides the 7-day slow push. **Distribution ledger & evidence pack** — a CSV ledger of every clip's source range (publish-link column left blank to fill) for authorization audits; optionally stream-copy ±3 minutes of source around each clip as context evidence. **AIGC labeling helper** — per-platform label copy and where the toggle lives; a cap hint when one stream yields >5 clips (flooding from a single stream is itself a repost signal). **Anti-fingerprint variants** — the last variant now swaps in a flash-forward opening (variants differ even in structure), and seeded template jitter (font/baseline micro-shifts) keeps batch output from sharing one production fingerprint. **Keep breaths** — jump cuts can keep a 0.25s natural pause instead of wall-to-wall speech.

<details>
<summary><b>Release history</b> (v0.4.3 → v0.13.0) and milestones</summary>
