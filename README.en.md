...
- **[v0.9.4](https://github.com/xixihhhh/hotclip/releases/tag/v0.9.4)** (2026-07-31) "Chinese usernames are off the hook": fixes **guaranteed transcription failure under non-ASCII Windows usernames** ([#4](https://github.com/xixihhhh/hotclip/issues/4)) — the speech engine's native layer opens files via ANSI, so with models under `C:\Users\中文名\...` the path gets mangled by the system codepage, nothing can be read, and all you got was "Please check your config!"; model paths are now auto-converted to 8.3 short paths and audio samples are read app-side into memory, so non-ASCII paths just work. Also fixes **cross-drive model moves being wrongly blocked** (same [#4](https://github.com/xixihhhh/hotclip/issues/4)) — any target on another drive was misjudged as "inside the current model folder", cutting off the escape route of moving models off C:; and if model loading still fails, the error now says **what to actually do** (move to an ASCII-only path / delete and re-download) instead of gibberish
- **[v0.9.3](https://github.com/xixihhhh/hotclip/releases/tag/v0.9.3)** (2026-07-28) "You decide where things live and how big they get": a new **Settings page** ([#3](https://github.com/xixihhhh/hotclip/issues/3)) — **model storage finally visible**: full path, all nine models itemized (purpose / disk usage / download size when missing) and a running total, with "open folder" and a **move-to-another-drive** action (instant same-disk rename; across drives it copies, verifies, and only then removes the original — a failed move leaves the original untouched); **three export quality tiers** (High / Standard / Compact — Compact measured 66% smaller, affecting sharpness only, never the edit); **default caption style** made explicit (it was always remembered, just never shown); and the export location moved in alongside them
- **[v0.9.2](https://github.com/xixihhhh/hotclip/releases/tag/v0.9.2)** (2026-07-27) "Honest errors, unsquashed UI": **transcription failures stop blaming your footage** ([#2](https://github.com/xixihhhh/hotclip/issues/2)) — every failure used to read "make sure the file has an audio track", whether the model wasn't installed, the disk was full, or the file really had no audio, sending people off to re-encode for nothing; failures are now re-probed and attributed (no audio track / model download failed / other), with the raw error shown verbatim for anything else. **The export-options bar no longer bursts the layout** ([#3](https://github.com/xixihhhh/hotclip/issues/3)) — with a dozen-plus toggles on one line, chips were squashed narrower than their own text and pushed a horizontal scrollbar onto the whole page; the bar now wraps, never squashes at any window width, and the sticky action bar is opaque so content below no longer bleeds through. **The output folder is configurable** (also [#3](https://github.com/xixihhhh/hotclip/issues/3)) — where clips land is spelled out next to the export button, "Change" picks a new folder and remembers it, and the watch folder follows the same setting
- **[v0.9.1](https://github.com/xixihhhh/hotclip/releases/tag/v0.9.1)** (2026-07-24) "Windows first-run fix": fixes the Windows first-run loop where the speech-model download hit 100%, restarted from zero and never finished ([#17](https://github.com/xixihhhh/clipforge/issues/17)) — Windows' built-in tar can't decompress bzip2, so the fully-downloaded archive failed to extract, was mistaken for corrupt, deleted, and re-downloaded from the next mirror forever; a pure-JS extractor now takes over automatically when system tar fails, so one download installs. Extraction also gets **its own progress stage** (unpacking a 1GB model takes minutes and no longer looks frozen), the size hint shows the real archive size (previously hard-coded 170MB), and extraction lands atomically via a staging dir (quitting mid-extract can no longer leave a half-installed model behind)
- **[v0.9.0](https://github.com/xixihhhh/hotclip/releases/tag/v0.9.0)** (2026-07-24) "Learns your taste, works out of the box": **review feedback loop** (which candidates you keep or reject at export time lands in a local preference file; the next detection run is steered toward "more like kept, fewer like rejected" — the more you use it, the better it knows your taste; collected on desktop, shared read-only by CLI/MCP/watch folder, and the data never leaves your machine), a **doctor self-check** (`pnpm cli doctor` inspects ffmpeg, all nine models with install state, the LLM endpoint, disk space and caches in one command with actionable fixes; `--download` pre-fetches the default-pipeline models), **resumable model downloads** (the 1GB ASR model survives dropped connections: resumes across mirrors and across restarts, with corrected size labels so the progress bar is finally honest), **structured post-copy templates** (8 hook angles × 5 CTA types drive the generated post copy with auditable angle/ctaType tags, CTA lines linted for banned words too), and the **reference-clip entrance completed** (MCP tools gain a `referencePath` param; the desktop highlight picker gets a "reference clip" button with a visible profile row)
- **[v0.8.0](https://github.com/xixihhhh/hotclip/releases/tag/v0.8.0)** (2026-07-20) "Clip like the viral one, repair itself": **reference-clip driven detection** (hand in a viral clip to model after — its duration / speech rate / shot rhythm / hook shape are measured locally and steer selection, CLI `--reference`), a **self-repairing QA loop** (fixable warnings get fixed on the spot: leading/trailing silence & black frames auto-trimmed, loudness re-normalized — kept only when the re-check improves, recorded in `qa.repair`), a **platform banned-words lint** (120+ local rules over titles / post copy / captions — absolute claims, medical claims, off-platform funneling flagged before you publish, zero upload), **Hormozi-style impact captions** (big bold chunks with hard shadows, word-by-word lighting), and **contact-sheet frame scoring** (9 numbered frames per grid — VLM calls drop 20→3 while the frame budget grows 20→27; the clip review modal gets the same at-a-glance frame overview)
...
- **Product mode (live-selling clips)**: type in a few product words and segment picking follows conversion logic — **live demos > selling-point pitches > price mechanics**, call-to-action lines kept only as natural endings next to a link drop, pure stalling-for-engagement segments (platforms flag them as violations) excluded; matched product words are deterministically merged into clip keywords and shown as 🛒 tags on candidate cards, so you can see at a glance which clip pitches which product; product words are remembered locally and compose with the hotword glossary
...
- **Dual aspect ratios in one click (vertical for Douyin, landscape for Bilibili)**: one toggle exports a landscape original-aspect version of the whole batch alongside the vertical 9:16 clips, into a subfolder — platform ratio specs used to mean a second editing pass; the landscape variant drops the title card (a vertical-shorts convention), captions switch to the smaller bottom layout, and covers/SRT/receipts come per variant; portrait sources skip it automatically and the progress bar counts doubled units. Opus/Vizard only export one ratio per run and resize clip by clip — one-click dual-ratio batch export is a first
...
Upgrade to [v0.9.1](https://github.com/xixihhhh/hotclip/releases) or newer. Older versions on Windows failed to extract the finished download (Windows' built-in tar lacks bzip2), mistook it for a corrupt file and re-downloaded from the next mirror in a loop. v0.9.1 ships a built-in extractor fallback; after upgrading, previously downloaded bytes resume automatically — no bandwidth wasted.
